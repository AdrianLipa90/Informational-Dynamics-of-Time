...
| Temporal Transport | `STRUCTURAL_REFERENCE_GATE_PASS / WAVE_ENERGY_TARGETED_PASS_CANDIDATE` | ordered interrupted propagation retained; 05B derives Cayley smooth segments from the Temporal Wave generator and a shared energy-metric contraction subclass |
| Memory | `ACTIVE_DERIVATION_TARGET` | event-driven Kepler branch, CP1/CPn Kähler frames, identifiable central parameter, append-only persistence and recall candidate |
...
| Retrodiction | `PROVISIONAL_DOWNSTREAM / GATED_PENDING_MEMORY_ORCHORBITAL_ADMISSION` | tested inverse-problem stack is staged on main; canonical admission follows the Memory -> ORCHORBITAL dependency path |
...
Dependency enforcement is one-way. Memory coordinates and recall operators are admitted inside the Memory node. ORCHORBITAL consumes the temporal-memory phase state and organizes it into typed attractor/orbit observables while the upstream definitions of time, NOW, bifurcation and temporal transport remain inherited through the graph. Retrodictive estimators consume outputs admitted by the Memory/ORCHORBITAL path. Spacetime quantities enter at their declared final downstream nodes.

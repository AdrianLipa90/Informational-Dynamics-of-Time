...
The machine-readable canonical dependency graph is maintained in `validation/dependency_graph.json`; `formalism/DEPENDENCY_GRAPH.md` is its human-readable projection.
...
Einstein closure also has the parallel prerequisite
...
The phase-clock/curvature interface is carried by 01K/01L, and the executable source side by 01AC/01AG. RFC RF-M1/RF-E0 supplies the hardened Maxwell/Bianchi bridge. The final Einstein gate is therefore reached only after both:

1. the temporal spine reaches admitted Retrodiction and retrocausal-test prerequisites; and
2. the parallel relativistic source bridge remains satisfied.

Current machine graph: `IDT_FORMAL_DEPENDENCY_GRAPH_V0_26`.

...
The NOW layer already supplies a non-negative gauge-invariant event magnitude \(q_n\). Using that amplitude and the projected event imprint, define the normalized localized memory action
\[
\boxed{
S_n^{(M)}(m)
...
\]
Integrating the Euler--Lagrange equation through one event gives the structural jump
\[
\boxed{
...
\]
Thus the previously open impulse interface now has an upstream-driven reference closure: event magnitude determines kick amplitude and the projective imprint determines direction.
...
The numerical reference records the Kepler--Newton controls in \texttt{validation/KEPLER\_MEMORY\_DYNAMICS\_V0\_1.json} and the event-action jump controls in \texttt{validation/EVENT\_MEMORY\_KICK\_V0\_1.json}. Physical-unit calibration is assigned to the later metric-time calibration stage. The next Memory-node derivation target is the origin of \(\mu_M\) and the persistence/recall law required to open retrodiction.

...
def _finite_hex(value: float, name: str, *, positive: bool = False, nonnegative: bool = False) -> str:
    if isinstance(value, (bool, np.bool_)):
        raise RetrodictionResidenceConditioningError(f"{name} must be a finite scalar")
    x = float(value)
    if not math.isfinite(x):
        raise RetrodictionResidenceConditioningError(f"{name} must be a finite scalar")
    if positive and x <= 0.0:
        raise RetrodictionResidenceConditioningError(f"{name} must be strictly positive")
    if nonnegative and x < 0.0:
        raise RetrodictionResidenceConditioningError(f"{name} must be non-negative")
    return x.hex()


def _event_fields(receipt: MemoryEventReceipt) -> tuple[str, str, str, str]:
    if not isinstance(receipt, MemoryEventReceipt):
        raise RetrodictionResidenceConditioningError("memory receipt must be a MemoryEventReceipt")
...
def _cell_payload(cell: MemoryORCHORBITALResidenceCell) -> dict[str, object]:
    return {
...
def _validate_cell(cell: MemoryORCHORBITALResidenceCell) -> None:
    if not isinstance(cell, MemoryORCHORBITALResidenceCell):
        raise RetrodictionResidenceConditioningError("cell must be a MemoryORCHORBITALResidenceCell")
    if type(cell.index) is not int or cell.index < 0:
        raise RetrodictionResidenceConditioningError("cell index must be a non-negative integer")
    for name, value in (
        ("memory_delta_tau_hex", cell.memory_delta_tau_hex),
        ("memory_event_weight_hex", cell.memory_event_weight_hex),
        ("memory_delta_m_real_hex", cell.memory_delta_m_real_hex),
        ...
        if type(value) is not str:
            raise RetrodictionResidenceConditioningError(f"{name} must be an exact float hex string")
        try:
            parsed = float.fromhex(value)
        except ValueError as exc:
            raise RetrodictionResidenceConditioningError(f"{name} is invalid") from exc
        if not math.isfinite(parsed):
            raise RetrodictionResidenceConditioningError(f"{name} must decode to a finite scalar")
    if float.fromhex(cell.memory_delta_tau_hex) <= 0.0:
        raise RetrodictionResidenceConditioningError("memory delta_tau must be strictly positive")
    if float.fromhex(cell.memory_event_weight_hex) < 0.0:
        raise RetrodictionResidenceConditioningError("memory event_weight must be non-negative")
    if not _is_sha256(cell.state_before_event_sha256):
        raise RetrodictionResidenceConditioningError("state_before_event_sha256 must be lowercase SHA-256")
    if cell.previous_cell_sha256 is not None and not _is_sha256(cell.previous_cell_sha256):
        raise RetrodictionResidenceConditioningError("previous_cell_sha256 must be null or lowercase SHA-256")
    if not _is_sha256(cell.cell_sha256):
        raise RetrodictionResidenceConditioningError("cell_sha256 must be lowercase SHA-256")
    try:
    ...
    if cell.residence_receipt.index != 0 or cell.residence_receipt.previous_receipt_sha256 is not None:
        raise RetrodictionResidenceConditioningError("event-local residence receipt must be a genesis segment")
    if cell.residence_receipt.delta_tau_hex != cell.memory_delta_tau_hex:
        raise RetrodictionResidenceConditioningError("memory and smooth-segment delta_tau disagree")
    if _cell_hash(cell) != cell.cell_sha256:
        raise RetrodictionResidenceConditioningError("event-residence cell content hash mismatch")
...
def build_memory_orchorbital_residence_cells(
    ...
    if not memory_receipts:
        raise RetrodictionResidenceConditioningError("memory_receipts must be non-empty")
    ...
    for index, event in enumerate(memory_receipts):
        ...
        try:
            bridge_cell, step = memory_orchorbital_cycle_forward(current, attractors, event)
            kicked = apply_receipt_kick(current, event)
            residence = residence_receipt_from_step(step, index=0, previous_receipt_sha256=None)
        except (ValueError, ORCHORBITALError) as exc:
            raise RetrodictionResidenceConditioningError(str(exc)) from exc
        if bridge_cell.active_attractor != residence.active_attractor:
            raise RetrodictionResidenceConditioningError("Memory/ORCH active snapshot disagrees with residence segment")
        if state_sha256(kicked) != residence.state_before_sha256:
            raise RetrodictionResidenceConditioningError("Memory event does not bridge to the committed smooth-segment state")
        draft = MemoryORCHORBITALResidenceCell(
            index=index,
            memory_delta_tau_hex=event_fields[0],
            memory_event_weight_hex=event_fields[1],
            memory_delta_m_real_hex=event_fields[2],
            memory_delta_m_imag_hex=event_fields[3],
...
def verify_memory_orchorbital_residence_cells(
    ...
    if not cells:
        raise RetrodictionResidenceConditioningError("cells must be non-empty")
    previous_hash: str | None = None
    previous_state_after: str | None = None
    for index, cell in enumerate(cells):
        _validate_cell(cell)
        if cell.index != index:
            raise RetrodictionResidenceConditioningError("cell indices must be contiguous from zero")
        if cell.previous_cell_sha256 != previous_hash:
            raise RetrodictionResidenceConditioningError("event-residence cell hash chain is broken")
        if previous_state_after is not None and cell.state_before_event_sha256 != previous_state_after:
            raise RetrodictionResidenceConditioningError("event-residence pre-event state lineage is discontinuous")
        previous_hash = cell.cell_sha256
...
def residence_lineage_signature(
    ...
    return ResidenceLineageSignature(
        active_sequence=tuple(cell.residence_receipt.active_attractor for cell in cells),
        next_sequence=tuple(cell.residence_receipt.next_attractor for cell in cells),
        switch_indices=tuple(cell.index for cell in cells if cell.residence_receipt.switched_after_segment),
        leak_indices=tuple(cell.index for cell in cells if cell.residence_receipt.post_segment_leak),
        winding_increments=tuple(cell.residence_receipt.winding_increment for cell in cells),
...
def _finite_kicks(kicks: Sequence[complex]) -> tuple[complex, ...]:
    if not kicks:
        raise RetrodictionResidenceConditioningError("kicks must be non-empty")
    out: list[complex] = []
    for raw in kicks:
        value = complex(raw)
        if not (math.isfinite(value.real) and math.isfinite(value.imag)):
            raise RetrodictionResidenceConditioningError("kicks must be finite")
        out.append(value)
    return tuple(out)
...
def _cells_for_kicks(
    ...
    if len(delta_taus) != len(kick_values):
        raise RetrodictionResidenceConditioningError("delta_taus must match the kick count")
    receipts = []
    for delta_tau, kick in zip(delta_taus, kick_values):
        dt = float(delta_tau)
        if not math.isfinite(dt) or dt <= 0.0:
            raise RetrodictionResidenceConditioningError("delta_taus must be finite and strictly positive")
        receipts.append(MemoryEventReceipt(dt, 1.0, kick))
    return build_memory_orchorbital_residence_cells(initial_state, attractors, receipts)
...
def audit_known_null_residence_conditioning(
    ...
    *,
    ...
    """Audit discrete residence/switch conditioning for one declared global-null pair.

    Semantic status uses only retained active labels and switch/leak lineage. Content
    hashes are provenance commitments and are intentionally excluded from the
    separation decision. Winding is reported independently as a continuous
    diagnostic coordinate.
    """
    tol = float(equivalence_tolerance)
    if not math.isfinite(tol) or tol <= 0.0:
        raise RetrodictionResidenceConditioningError("equivalence_tolerance must be finite and strictly positive")
    ref = _finite_kicks(reference_kicks)
    alt = _finite_kicks(alternate_kicks)
    if len(ref) != len(alt):
        raise RetrodictionResidenceConditioningError("reference and alternate histories must have equal event count")
    latent_separation = float(np.linalg.norm(_latent_vector(ref) - _latent_vector(alt)))
    if latent_separation <= tol:
        raise RetrodictionResidenceConditioningError("known-null candidates must be distinct in latent coordinates")
    try:
        base_ref = sparse_orchorbital_observation(
            initial_state, attractors, delta_taus, ref, base_observations
        )
        base_alt = sparse_orchorbital_observation(
            initial_state, attractors, delta_taus, alt, base_observations
        )
    except ValueError as exc:
        raise RetrodictionResidenceConditioningError(str(exc)) from exc
    ...
    ref_signature = residence_lineage_signature(
        _cells_for_kicks(initial_state, attractors, delta_taus, ref)
    )
    alt_signature = residence_lineage_signature(
        _cells_for_kicks(initial_state, attractors, delta_taus, alt)
    )
    active_equal = ref_signature.active_sequence == alt_signature.active_sequence
    switch_equal = (
    ...
    winding_ref = np.asarray(ref_signature.winding_increments, dtype=float)
    winding_alt = np.asarray(alt_signature.winding_increments, dtype=float)
    winding_residual = float(np.linalg.norm(winding_ref - winding_alt))
    ...
    return ResidenceConditioningAudit(
        ...
        provenance_heads_equal=(
            ref_signature.bridge_head_sha256 == alt_signature.bridge_head_sha256
        ),
